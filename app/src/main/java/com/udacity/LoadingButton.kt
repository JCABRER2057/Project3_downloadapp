...
private const val ANIMATION_DURATION: Long = 10000
...
class LoadingButton @JvmOverloads constructor(
    ...
    private fun setDefaultButtonState(canvas: Canvas?) {
        paint.color = textColor
        paint.textAlign = Paint.Align.CENTER
        canvas?.drawText(context.getString(R.string.button_text_download), width / 2.0f, height / 2.0f, paint)
    }
    ...
    private fun setAnimator() {
        ...
        valueAnimator.addUpdateListener {
            ...
            //the invalidate() method invalidates the entire view, forcing a call to onDraw() to redraw
            invalidate()
        }
        valueAnimator.start()
    }
